...
public class PlayerController : MonoBehaviour
{
    ...
    private void HandleControls()
    {
        ...
        if (Input.GetKey(KeyCode.Space) && jumpTime > Time.time - fTime && bGrounded) //when space is pressed
        { 
            playerMove.y = jumpHeight * Mathf.Sin(Mathf.PI * ((Time.time - fTime) / jumpTime));
            Debug.Log("jump");
        }
        else
        {
